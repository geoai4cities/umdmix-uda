...
class SemanticPOSSDataset(BaseDataset):
    def __init__(
        ...
        version="full",
        phase="train",
        dataset_path="/data4/vaibhav/lidardata/SemanticPOSS_dataset/dataset/sequences",
        ...
        if self.version == "full":
            self.split = {"train": ["00", "01", "02", "04", "05"], "validation": ["03"]}
        elif self.version == "mini":
            self.split = {"train": ["00", "05"], "validation": ["03"]}
        else:
        ...
        # print(f"semanticposs, max key\t", max_key)
        ...
        for sequence in self.split[self.phase]:
            ...
            for f in np.arange(num_frames):
                pcd_path = os.path.join(
                    self.dataset_path, sequence, "velodyne", f"{int(f):06d}.bin"
                )
                label_path = os.path.join(
                    self.dataset_path, sequence, "labels", f"{int(f):06d}.label"
                )
