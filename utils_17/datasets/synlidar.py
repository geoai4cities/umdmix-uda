...
class SynLiDARDataset(BaseDataset):
    def __init__(
        ...
        version: str = "full",
        phase: str = "train",
        dataset_path: str = "/data3/vaibhav/SynLiDAR/sequences/",
        ...
        # print(
        #     f"SynLiDAR max_key", max_key, remap_dict_val.keys(), remap_dict_val.values()
        # )
        remap_lut_val = np.zeros((max_key + 100), dtype=np.int32)
        # print("Synlidar remap_lut_val", np.zeros((max_key + 100), dtype=np.int32).shape)
        remap_lut_val[list(remap_dict_val.keys())] = list(remap_dict_val.values())
        # print(f"remap_lut_val:\n", remap_lut_val)
        ...
        for sequence, frames in self.split[self.phase].items():
            for frame in frames:
                pcd_path = os.path.join(
                    self.dataset_path, sequence, "velodyne", f"{int(frame):06d}.bin"
                )
                label_path = os.path.join(
                    self.dataset_path, sequence, "labels", f"{int(frame):06d}.label"
                )
    ...
    def get_splits(self):
        ...
        if not os.path.isfile(split_path):
            self.split = {
                "train": {s: [] for s in self.sequences},
            ...
            if self.version != "sequential":
                for sequence in self.sequences:
                    ...
                    for v in np.arange(num_frames):
                        pcd_path = os.path.join(
                            self.dataset_path, sequence, "velodyne", f"{int(v):06d}.bin"
                        )
                        label_path = os.path.join(
                            self.dataset_path, sequence, "labels", f"{int(v):06d}.label"
                        )

                        if os.path.isfile(pcd_path) and os.path.isfile(label_path):
                            valid_frames.append(v)
                    if self.version == "full":
                        train_selected = np.random.choice(
                            valid_frames, int(num_frames / 10), replace=False
                        )
                    else:
                        train_selected = np.random.choice(
                            valid_frames, int(num_frames / 100), replace=False
                        )

                    for t in train_selected:
                        valid_frames.remove(t)

                    validation_selected = np.random.choice(
                        valid_frames, int(num_frames / 100), replace=False
                    )
                ...
                for sequence in self.sequences:
                    ...
                    for v in np.arange(num_frames):
                        ...
                        if os.path.isfile(pcd_path) and os.path.isfile(label_path):
                            if len(train_selected) == 0:
                                ...
                                len(train_selected) < total_for_sequence
                                and (v - last_added) >= 5
                            ):
                                train_selected.append(v)
                                last_added = v
                    ...
                    validation_selected = np.random.choice(
                        validation_selected, int(num_frames / 100), replace=False
                    )
